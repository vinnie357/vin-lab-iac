...
data "template_file" "metadata" {
  template = file("${path.root}/vsphere/templates/f5/nginx/nim/metadata.yml")
  vars = {
    HOST = var.vm_name
  }
}
data "template_file" "userdata" {
  template = file("${path.root}/vsphere/templates/f5/nginx/nim/userdata.yml")
  vars = {
...
data "template_file" "kickstart" {
  template = file("${path.root}/vsphere/templates/f5/nginx/nim/kickstart.yml")
  vars = {
    USER   = var.adminUser
    PASS   = var.adminPass
