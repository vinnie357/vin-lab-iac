...
#
# download latest DO
files=$(curl -s  https://api.github.com/repos/F5Networks/f5-declarative-onboarding/releases/latest | grep "browser_download_url.*rpm" | cut -d : -f 2,3 | tr -d \")
for file in $files
do
  name=$(echo "${file##*/}")
...
# download latest As3
files=$(curl -s  https://api.github.com/repos/F5Networks/f5-appsvcs-extension/releases/latest | grep "browser_download_url.*rpm" | cut -d : -f 2,3 | tr -d \")
for file in $files
do
  name=$(echo "${file##*/}")
  result=$(/usr/bin/curl -kvv -w "%{http_code}" $file -o /var/config/rest/downloads/$name)
...
# download latest ts
files=$(curl -s  https://api.github.com/repos/F5Networks/f5-telemetry-streaming/releases/latest | grep "browser_download_url.*rpm" | cut -d : -f 2,3 | tr -d \")
for file in $files
do
  name=$(echo "${file##*/}")
  result=$(/usr/bin/curl -kvv -w "%{http_code}" $file -o /var/config/rest/downloads/$name)
...
function checkService() {
    status=$(/usr/bin/curl -skv -w "%{http_code}" -u azureuser: -o /dev/null  https://$host:$dfl_mgmt_port$1)
    echo $status
}
# set status
